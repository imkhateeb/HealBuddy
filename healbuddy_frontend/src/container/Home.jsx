...
export default function Home() {
  ...
  return (
    <div className='flex bg-mainColor flex-col w-screen h-screen transition-height duration-75 ease-out'>
      ...
      <div className='flex md:hidden flex-col'>
        ...
        {toggleDashboard && (
          <div className='fixed left-0 top-0 w-4/5 bg-white h-screen overflow-y-auto shadow-md z-10 animate-fade-in flex flex-col'>
            ...
            <Dashboard user={user && user} ole={(role === "expert") || ( role === "client") ? role : ""} closeToggle={setToggleDashboard} />
          </div>
        )}
        ...
        {toggleSidebar &&
          <div className='fixed top-0 right-0 w-4/5 bg-white h-screen overflow-y-auto shadow-md z-10 animate-fade-in flex'>
            ...
            <Sidebar user={user && user} ole={(role === "expert") || ( role === "client") ? role : ""} closeToggle={setToggleSidebar} />
          </div>
        }
        ...
        <div className='pb-2 flex-1 h-screen overflow-y-scroll' ref={scrollRef}>
          ...
          <Routes>
            <Route path='/*' element={<HomeDisplay user={user && user} ole={(role === "expert") || ( role === "client") ? role : ""} />} />
          </Routes>
        </div>
      </div>
    </div>
