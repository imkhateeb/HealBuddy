...
export default function Sidebar({ user, role }) {
  ...
  const isNoteActiveStyle = 'flex flex-col py-1 pl-5 gap-3 text-gray-500 hover:text-black transition-all duration-200 ease-in-out capitalize my-1';
  const isActiveStyle = 'flex flex-col py-2 gap-3 font-extrabold border-r-2 border-black transition-all duration-200 ease-in-out capitalize my-2 pl-2';
  ...
  return (
    <div className='flex flex-col bg-mainColor h-full min-w-210 hide-scrollbar'>
      ...
      <div className='border-gray-500 flex flex-col'>
        ...
        <NavLink to={'/services'} className={({ isActive }) => isActive ? isActiveStyle : isNoteActiveStyle}><GrServices fontSize={25} />Services</NavLink>
        ...
        {role === "expert" && (
          <>
            ...
            <NavLink to={`/expert/${user?._id}/pings`} className={({ isActive }) => isActive ? isActiveStyle : isNoteActiveStyle}><BsFillPinFill fontSize={25} />Pings</NavLink>
          </>
        )} {role === "client" && (
          <NavLink to={'/ai-chatbot'} className={({ isActive }) => isActive ? isActiveStyle : isNoteActiveStyle}><BsWechat fontSize={25} />24/7 Chat</NavLink>
        )}
